...
pub fn setup(
    ...
    let handle = asset_server.load("data/b_n_dark elf_m_skins.nif");
    ...
    let handle1 = asset_server.load("data/B_N_Dark Elf_M_Head_01.nif");
    //commands.spawn((NifScene(handle5), Transform::from_xyz(0.0, 0.0, 0.0)));
    spawn_gltf_as_child(
        handle1,
        &mut scene_entities_by_name,
    ...
    let handle6: Handle<Nif> = asset_server.load("data/Xbase_anim.kf");
    commands.spawn((
        NifScene(handle6),
        Transform::from_xyz(0.0, 0.0, 0.0),
        InheritedVisibility::VISIBLE,
